...
public class PostloginUI implements UI {
    ...
    private UI handleJoin(String[] parts) {
        ...
        try {
            ...
            GameData joinedGame = server.joinGame(game.gameID(), color);
            return new GamePlayUI(server, authData, game.gameID(), color, joinedGame.game());
        }
    ...
    private UI handleObserve(String[] parts) {
        ...
        try {
            ...
            return new GamePlayUI(server, authData, actualGameId, "OBSERVER");
        }
        catch (Exception e) {
